    ...
    unsigned int                    vertical_reduction )  /* the reduction in vertical scale */
{
  ...
  for (height_index = 0; height_index < scaled_height; height_index++)
  {
    ...
    {
	  if(unscaled_data[height_index*scaled_width*vertical_reduction*horizontal_reduction + width_index*horizontal_reduction].R < 30) {
		  scaled_data[height_index*scaled_width + width_index].R = 0;
		  scaled_data[height_index*scaled_width + width_index].G = 0;
		  scaled_data[height_index*scaled_width + width_index].B = 0;
	  }
	  else {
		  scaled_data[height_index*scaled_width + width_index].R = 255;
		  scaled_data[height_index*scaled_width + width_index].G = 255;
		  scaled_data[height_index*scaled_width + width_index].B = 255;
	  }
    }
